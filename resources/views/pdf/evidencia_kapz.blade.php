...
<head>
    ...
    <title>EVIDENCIA ODPRACOVANEJ DOBY</title>
    <style>
...
<body>
    ...
    <div class="doc-title">EVIDENCIA ODPRACOVANEJ DOBY</div>
    ...
    <table class="meta-table">
        ...
        <tr>
            ...
            <td><strong>{{ $kapz->full_name }}</strong></td>
            <td><strong>Os. č.:</strong></td>
            <td><strong>{{ $kapz->personal_number }}</strong></td>
            <td style="width: 7%;"><strong>Oblasť:</strong></td>
            <td style="width: 20%;">{{ $kapz->scope }}</td>
    ...
    @php
        ...
        $stdHours = ($kapz->employment_ratio ?? 1.0) * 7.5;
        ...
        <tbody>
            @for($d = 1; $d <= 31; $d++)
                ...
                <tr class="{{ $isWeekend ? 'weekend-row' : '' }}">
                    <td>{{ $d }}</td>
                    <td class="label-cell">{{ $isWork ? ($entry && $entry->workplace ? $entry->workplace : ($entry && $entry->activity_description ? $entry->activity_description : $kapz->scope)) : '' }}</td>
                    <td></td>
